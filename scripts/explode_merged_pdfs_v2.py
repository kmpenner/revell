    ...
    for folder_name in check_folders:
        ...
        for pdf_file in pdfs:
            ...
            if len(articles_found) > 1:
                ...
                doc.close()
                # Move original to a backup instead of deleting
                backup_dir = os.path.join(ROOT_DIR, "backups")
                os.makedirs(backup_dir, exist_ok=True)
                print(f"  Moving original to backup: {pdf_file}")
                try:
                    time.sleep(1) # Give handle release time
                    shutil.move(pdf_path, os.path.join(backup_dir, pdf_file))
                except Exception as e:
                    print(f"  Warning: Could not move {pdf_file} to backup: {e}")
            else:
                doc.close()
